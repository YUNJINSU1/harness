...
| [`imagegen`](../../skills/imagegen/SKILL.md) | 웹 섹션·모바일 앱 화면·브랜드 콘셉트 보드의 실제 이미지 생성과 시각 확인. 내부 참조는 `frontend/common`, `frontend/web`, `frontend/mobile`, `brandkit`이며 별도 등록 스킬이 아닙니다. |
...
네 스킬을 순서대로 모두 실행하는 파이프라인은 아닙니다. 작은 UI 수정에 이미지 생성이나 `DESIGN.md`를 요구하지 않습니다. 이미지 생성 단계의 코드 작성 금지는 이미지 인계에서 끝나며, `image-to-code`는 실제 구현과 실행 검증을 이어갑니다. 이미지의 경로·섹션/화면·primary/detail·viewport/플랫폼·보존/제안·알려진 문구/토큰/에셋·미확인 동작을 응답이나 기존 브리프로 전달하며 별도 manifest는 만들지 않습니다.

`imagegen`은 환경에 실제 제공된 이미지 도구를 확인해 사용합니다. 도구가 없으면 생성 실패 범위를 알리고 가짜 이미지·코드로 대신하지 않습니다. 정적 화면은 반응형·상호작용·접근성 준수의 증거가 아니며, 브랜드 보드는 벡터 로고·폰트 파일·상표 검토·인쇄용 원본을 뜻하지 않습니다. 일반 사진·다이어그램·슬라이드는 이 스킬의 UI/브랜드 범위와 구분합니다.
